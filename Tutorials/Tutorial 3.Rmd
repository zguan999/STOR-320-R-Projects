...
```{r,eval=F}
f1e<-arrange(f1d,desc(carrier,origin,destination))
head(f1e,18)
```
...
Why or why not?
flight numbers may map to multiple carriers 

If we know the flight number, carrier, and origin, can we know for sure what the destination will be? No (*Yes/No*)

Why or why not?
The origins go to multiple destinations even with the same carrier 
...
f2b=mutate(f2a,
           ...
           dep_arr_hr=arr_hr - sched_arr_hr)
```
...
```{r,eval=F}
f2c=mutate(f2b,
           gain_hr= dep_arr_hr - dep_delay_hr,
...
```{r,eval=F}
f2d<-filter(f2c, percent_gain_hr>.9 | percent_gain_hr<.1)
head(f2d)
```
